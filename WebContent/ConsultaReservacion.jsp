...
<%@page import="java.sql.*" %>
	...
	<jsp:include page="ConsultaReservacion.html"></jsp:include><br/>
	...
	if (evento == null){
		...
		queryVenta=SetQueryConsulta.getReservacion(evento, idUsuario, timeI, timeF);
		rs2 = getVenta.executeQuery(queryVenta);
	
	%>
	<form action="PagoReservacion.jsp" method="post">
	<table class="blueTable">
		...
		<% 
			...
			%>
			<tr>
				<td><input type="radio" name="idReservacion"
					value="<%=rs2.getInt(1) %>" id="idReservacion"><%=rs2.getInt(1) %></td>
				<td><input type="hidden" id="evento" name="evento" value="<%=rs2.getString(5) %>"><%=rs2.getString(5) %></td>
				<td><%=rs2.getString(6) %></td>
				<td><%=rs2.getInt(2) %></td>
				<td><%=rs2.getString(3) %></td>
				<td><%=rs2.getString(4) %></td>
				<td><input type="hidden" id="zona" name="zona" value="<%=rs2.getInt(7) %>"><%=rs2.getInt(7) %></td>
				<td><input type="hidden" id="asiento" name="asiento" value="<%=rs2.getInt(8) %>"><%=rs2.getInt(8) %></td>
			</tr>
